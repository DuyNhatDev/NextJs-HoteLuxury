...
import { useGetAccount, useUpdateProfileMutation } from '@/hooks/queries/useAccount'
import { UpdateProfileBodySchema, UpdateProfileBodyType } from '@/schemas/account.schema'
        ...
        <Card className='gap-0 rounded'>
          <CardHeader className='flex flex-row items-center justify-between border-b-8 border-gray-200 pb-4'>
            <div className='flex flex-col'>
              <CardTitle className='text-2xl font-bold text-blue-900'>Thông tin đối tác</CardTitle>
            </div>
                    ...
                    <FormItem>
                      {editingField === 'birthDate' ? (
                        ...
                        <p className={field.value ? '' : 'text-gray-500'}>
                          {field.value ? formatDate(new Date(field.value), 'dd-MM-yyyy') : 'Chọn ngày sinh'}
                        </p>
                      )}
                    </FormItem>
                  )}
